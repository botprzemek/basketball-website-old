<template>
  <section class="w-full h-fit flex flex-row flex-grow gap-3">
    <section class="w-full h-fit grid grid-flow-row gap-3 grid-cols-[3fr_repeat(2,_8fr)_2fr] mb-3">
      <div class="w-full h-full bg-main grid place-content-center">
      ...
      <section class="w-full h-fit px-3 py-2 border-secd border-[1px]">
        <input @input="passData" :id="`firstname-${id}`" v-model="player.firstName"
               ...
               pattern="[a-zA-Z]{3,24}" placeholder="Imię" required type="text">
      </section>
      <section class="w-full h-fit px-3 py-2 border-secd border-[1px]">
        <input @input="passData" :id="`lastname-${id}`" v-model="player.lastName"
               ...
               pattern="[a-zA-Z]{3,24}" placeholder="Nazwisko" required type="text">
      </section>
      <section class="w-full h-fit px-3 py-2 border-secd border-[1px]">
      <input @input="passData" :id="`age-${id}`" v-model="player.age"
             ...
             size="2" pattern="[0-9]{1,2}" placeholder="Wiek" required type="number">
      </section>
    </section>
